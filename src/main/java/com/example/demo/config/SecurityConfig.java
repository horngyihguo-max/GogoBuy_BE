...
import com.example.demo.service.GoogelOAuth2;
...
public class SecurityConfig {
	...
	@Autowired
    private GoogelOAuth2 googelOAuth2;
	
	// @BEAN 這是最重要的「過濾鏈」。所有進出網站的 HTTP 請求都要經過這道門。
	@Bean
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http
		/*跨來源資源共享 是一種瀏覽器機制，透過特定的 HTTP 標頭，
		 * 允許一個網域的網頁程式（如 JavaScript）安全地向不同網域的伺服器請求資源，*/
			.cors(withDefaults()) 
			.csrf(csrf -> csrf.disable())
				...
				.oauth2Login(oauth2 -> {
	                ...
	                oauth2.userInfoEndpoint(userInfo -> 
	                    userInfo.userService(googelOAuth2)
	                );
	                // 登入成功後的跳轉路徑
